...
def create_2d_cart(npoints, p1, P1, reorder):
   ...
   cart2d = comm.Create_cart(dims, periods=periods, reorder=reorder)    
   return dims, cart2d
...
def create_2dCoords(cart2d, npoints, dims):
   ...
   sx = coords[0]*npoints[0]//dims[0]
   ex = (coords[0]+1)*npoints[0]//dims[0]-1
   sy = coords[1]*npoints[1]//dims[1]
   ey = (coords[1]+1)*npoints[1]//dims[1]-1
...
def create_neighbours(cart2d):
   ...
   neighbour[N], neighbour[S] = cart2d.Shift(0, 1)
   ''' Get my western and eastern neighbours '''
   neighbour[W], neighbour[E] = cart2d.Shift(1, 1)
...
def create_derived_type(sx, ex, sy, ey):
   ...
   type_line = MPI.DOUBLE.Create_vector(ex-sx+1, 1, ey-sy+3)
   type_line.Commit()


   '''Creation of the type_column derived datatype to exchange points
   with western to eastern neighbours '''
   type_column = MPI.DOUBLE.Create_vector(ey-sy+1, 1, 1)
   type_column.Commit()
...
def communications(u, sx, ex, sy, ey, type_column, type_line):
   ''' Send to neighbour N and receive from neighbour S '''
   comm.Sendrecv(sendbuf=[u[IDX(sx, sy)], type_line], dest=neighbour[N], sendtag=0,
                 recvbuf=[u[IDX(ex+1, sy)], type_line], source=neighbour[S], recvtag=0)


   ''' Send to neighbour S and receive from neighbour N '''
   comm.Sendrecv(sendbuf=[u[IDX(ex, sy)], type_line], dest=neighbour[S], sendtag=0,
                 recvbuf=[u[IDX(sx-1, sy)], type_line], source=neighbour[N], recvtag=0)


   ''' Send to neighbour W and receive from neighbour E '''
   comm.Sendrecv(sendbuf=[u[IDX(sx, sy)], type_column], dest=neighbour[W], sendtag=0,
                 recvbuf=[u[IDX(sx, ey+1)], type_column], source=neighbour[E], recvtag=0)


   ''' Send to neighbour E  and receive from neighbour W '''
   comm.Sendrecv(sendbuf=[u[IDX(sx, ey)], type_column], dest=neighbour[E], sendtag=0,
                 recvbuf=[u[IDX(sx, sy-1)], type_column], source=neighbour[W], recvtag=0)
...
def initialization(sx, ex, sy, ey):
   ...
   for i in range(sx, ex+1):
       for j in range(sy, ey+1):
           f[IDX(i, j)] = 2*(i*hx*hx - i + j*hy*hy - j)
           u_exact[IDX(i, j)] = i*hx*j*hy*(1-i*hx)*(1-j*hy)
...
def computation(u, u_new):
   ...
   for i in range(sx, ex+1):
       for j in range(sy, ey+1):
           u_new[IDX(i, j)] = coef[0] * (coef[1]*(u[IDX(i+1, j)] + u[IDX(i-1, j)]) + coef[2]*(u[IDX(i, j+1)] + u[IDX(i, j-1)]) - 2*(i*hx*hx - i + j*hy*hy - j))
...
neighbor      = create_neighbours(cart2d)
...
while (not(convergence) and (iter < it_max)):
   iter = iter+1
   ...
   norm = comm.allreduce(np.array(local_error), op=MPI.MAX )   
  
   ''' Stop if we obtained the machine precision '''
   convergence = (norm < eps)
   
   ''' Print diffnorm for process 0 '''
   if ((rank == 0) and ((iter % 100) == 0)):
       print("Iteration", iter, " global_error = ", norm);
...
if (rank == 0):
   ...
   print("Convergence after",iter, 'iterations in', t2-t1,'secs')
